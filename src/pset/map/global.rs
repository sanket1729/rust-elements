...
impl Default for Global {    fn default() -> Self {
        Self {
...
impl Map for Global {
            ...
            PSET_GLOBAL_PROPRIETARY => {
                ...
                if prop_key.is_pset_key() && prop_key.subtype == PSBT_ELEMENTS_GLOBAL_SCALAR {
                    if raw_value.is_empty() && prop_key.key.len() == 32 {
                        ...
                        slice.copy_from_slice(&prop_key.key);
                        if !self.scalars.contains(&slice) {
                            self.scalars.push(slice);
                        } else {
    ...
    fn get_pairs(&self) -> Result<Vec<raw::Pair>, encode::Error> {
        ...
        for scalar in &self.scalars {
            let key = raw::ProprietaryKey::from_pset_pair(PSBT_ELEMENTS_GLOBAL_SCALAR, scalar.to_vec());
            rv.push(raw::Pair {
                key: key.to_key(),
                value: vec![],
            })
...
impl Decodable for Global {
    fn consensus_decode<D: io::Read>(mut d: D) -> Result<Self, encode::Error> {
        ...
        // Mandatory fields
        let version = version.ok_or(Error::MissingPsetVersion)?;
