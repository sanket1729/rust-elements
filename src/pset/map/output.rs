...
pub struct Output {
    ...
    pub value_rangeproof: Option<Vec<u8>>,
    /// Output Asset surjection proof
    pub asset_surjection_proof: Option<Vec<u8>>,
                ...
                if prop_key.is_pset_key() {
                    match prop_key.subtype {
                        ...
                        PSBT_ELEMENTS_OUT_VALUE_RANGEPROOF => {
                            impl_pset_insert_pair!(self.value_rangeproof <= <raw_key: _> | <raw_value : Vec<u8>>)
                        }
                        PSBT_ELEMENTS_OUT_ASSET_SURJECTION_PROOF => {
                            impl_pset_insert_pair!(self.asset_surjection_proof <= <raw_key: _> | <raw_value : Vec<u8>>)
                        }
