...
pub struct Input {
    ...
    pub issuance_value_rangeproof: Option<Vec<u8>>,
    /// Issuance keys rangeproof
    pub issuance_keys_rangeproof: Option<Vec<u8>>,
    ...
    pub issuance_blinding_nonce: Option<[u8; 32]>,
...
impl Input{
    ...
    pub fn from_txin(txin: TxIn) -> Self {
        ...
        if txin.has_issuance {
            ...
            // Witness
            ret.issuance_keys_rangeproof = Some(txin.witness.inflation_keys_rangeproof);
            ret.issuance_value_rangeproof = Some(txin.witness.amount_rangeproof);
        }
        ret
    }
}
                ...
                if prop_key.is_pset_key() {
                    match prop_key.subtype {
                        ...
                        PSBT_ELEMENTS_IN_ISSUANCE_VALUE_RANGEPROOF => {
                            impl_pset_prop_insert_pair!(self.issuance_value_rangeproof <= <raw_key: _> | <raw_value : Vec<u8>>)
                        }
                        PSBT_ELEMENTS_IN_ISSUANCE_KEYS_RANGEPROOF => {
                            impl_pset_prop_insert_pair!(self.issuance_keys_rangeproof <= <raw_key: _> | <raw_value : Vec<u8>>)
                        }
                        ...
                        PSBT_ELEMENTS_IN_ISSUANCE_BLINDING_NONCE => {
                            impl_pset_prop_insert_pair!(self.issuance_blinding_nonce <= <raw_key: _> | <raw_value : [u8;32]>)
                        }
...
impl Decodable for Input {
    fn consensus_decode<D: io::Read>(mut d: D) -> Result<Self, encode::Error> {
        ...
        let prev_txid = prev_txid.ok_or(Error::MissingPsetVersion)?;
